...
function WeatherCallback(response){
  ...
  if (response.main && response.weather && response.weather.length) {
    weatherString = response.name + ': ' + response.main.temp + ' degrees F, and ' + response.weather[0].description;
  }
...
function OneShotTimeCallback(startTime){
  var oneShotString = 'Called after ' + Math.trunc((performance.now()-startTime)*1000)/1000 + ' milliseconds';
